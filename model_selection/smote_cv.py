...
def k_fold_smote_regressor(train_x, train_y, test_x, model_num, cv=5, important_level=2):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(train_x)):
        ...
        high_num = X_y_train[X_y_train['class'] == 0]
        median_num = X_y_train[X_y_train['class'] == 2]
        low_num = X_y_train[X_y_train['class'] == 1]

        high_scale = np.floor(median_num/high_num)
        low_scale = np.floor(median_num/low_num)

        high_data = X_y_train[X_y_train['class'] == 0].iloc[:, :-1]
        low_data = X_y_train[X_y_train['class'] == 1].iloc[:, :-1]
        median_data = X_y_train[X_y_train['class'] == 2].iloc[:, :-1]

        high_samples = []
        if high_scale >= 2:
            high_samples = [high_data] +[create_sample(high_data) for time in high_scale-1]

        low_samples=[]
        if low_scale >= 2:
            low_samples = [low_data] + [create_sample(low_data) for time in low_scale-1]

        print('high:' + str(len(high_samples)) + ' median:' + str(len(median_data)) + ' low:' + str(len(low_samples)))

        new_data = pd.concat([high_samples, median_data, low_samples], axis=0)
