...
def k_fold_validation(train_x, train_y, test_x, model_num, cv=5):
    print('开始CV 5折训练...')
    ...
    for i, (train_index, test_index) in enumerate(kf.split(train_x)):
        ...
        model = mf.create_model(model_num)
    ...
    print(mf.get_model_name(model_num) + ' k fold validation:', sum(mses)/len(mses))
...
def save_prediction(pred, model_num):
    ...
    b_pattern['Y'] = pred
    b_pattern.to_csv('output/' + mf.get_model_name(model_num) + '_pred.csv', index=None, header=None)